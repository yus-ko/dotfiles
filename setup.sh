...
if confirm "必要パッケージと日本語ロケールをセットアップしますか?"; then
  ...
  if command -v apt-get &>/dev/null; then
    # Debian / Ubuntu
    APT_PACKAGES=(zsh git curl tmux fzf unzip bat locales tzdata)
    ROOT=()
    ...
    if [ ${#MISSING[@]} -gt 0 ]; then
      ...
      for pkg in "${MISSING[@]}"; do
        if apt-cache show "$pkg" &>/dev/null; then
          ...
          echo "  - $pkg はAPTで提供されていないためHomebrewで補完します"
        fi
      done
      MISSING=("${INSTALLABLE[@]}")
    fi
...
if [ -t 0 ] && [ "$(basename "$SHELL")" != "zsh" ] && command -v zsh &>/dev/null; then
  echo "zsh を起動します..."
  exec zsh -l
fi
