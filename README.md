...
セットアップスクリプトで以下のツールがインストールされます（各ツール個別に確認あり）：
...
cd ~/dotfiles
./setup.sh        # ステップごとに確認しながらインストール
./setup.sh -y     # 確認なしで全インストール
./setup.sh -h     # ヘルプ表示
```
...
4. zsh プラグインのインストール
5. Oh My Tmux のインストール
6. 各設定ファイルのシンボリックリンク作成（既存ファイルは `tmp/` にバックアップ）
7. Neovim セットアップ（LazyVim starter + カスタマイズファイル）
8. zsh-abbr の略語設定の配置
9. オプショナルCLIツールのインストール
10. デフォルトシェルを zsh に変更
11. zsh 以外で実行した場合、終了後に zsh を起動
